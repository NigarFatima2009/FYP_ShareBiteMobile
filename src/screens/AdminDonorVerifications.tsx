...
interface AdminDonorVerificationsProps { navigation: any; }

export const AdminDonorVerifications: React.FC<AdminDonorVerificationsProps> = ({ navigation }) => {
    ...
    const handleApprove = (app: any) => {
        Alert.alert('Approve Donor', `Verify ${app.fullName}?`, [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Approve', onPress: async () => {
                    ...
                    try {
                        ...
                        batch.update(firestore().collection('donorVerification').doc(app.id), { status: 'approved', verified: true, verifiedAt: firestore.FieldValue.serverTimestamp() });
                        batch.update(firestore().collection('users').doc(app.id), { verificationStatus: 'approved', donorVerified: true });
                        batch.set(firestore().collection('notifications').doc(), { userId: app.id, title: 'Verified!', message: 'Your donor account is verified.', type: 'verification_approved', read: false, createdAt: firestore.FieldValue.serverTimestamp() });
                        await batch.commit();
    ...
    const handleReject = async () => {
        ...
        try {
            const batch = firestore().batch();
            batch.update(firestore().collection('donorVerification').doc(selectedApp.id), { status: 'rejected', rejectionReason, updatedAt: new Date().toISOString() });
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Donor Verifications</Text>
            </View>

            <ScrollView contentContainerStyle={styles.content}>
                {applications.length === 0 ? <Text style={styles.empty}>No pending donor applications</Text> : applications.map(app => (
                    <Card key={app.id} style={styles.card}>
                        ...
                        <Text style={styles.subText}>CNIC: {app.cnic}</Text>
                        <Text style={styles.subText}>Address: {app.address}</Text>
            ...
            <Modal visible={rejectionModalVisible} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        <Text style={styles.modalTitle}>Reject Donor</Text>
